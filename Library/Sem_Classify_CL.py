...
#		XValid_3 = np.fliplr(XValid_3)
		...
		wMod = gensim.models.Word2Vec.load('Sem_WV.bin')
		w2v = dict(zip(wMod.wv.index2word, wMod.wv.syn0))

		#Populating the GloVe based representation for words in the dataset
		vocabulary_size = 19999#len(XUni)
		embedding_matrix = np.zeros((vocabulary_size+1, 50))
		for word in XUni:
			Idx = W2I[word]
...
#		ELayer = Embedding(input_dim=iDim, output_dim=Edim, input_length=actStep, mask_zero=False, weights=[embedding_matrix], trainable=True, embeddings_constraint=unit_norm())
		...
		model = load_model('SemMod_ED.h5', custom_objects={'AttLayer2' :  AttLayer2})		
